# app.py — BookClimbing (전체 버전) / GitHub 동기화 + 자동 DB 저장
# PART 1/2
...
:root{{ --bg:#ffffff; --sidebar-bg:#f6f7fb; --card:#ffffff; --text:#0b1220; --muted:#4b5563; --ring:#e5e7eb;
...
def _gh_get_contents(path):
    ...
    if r.status_code == 200:
        ...
        raise RuntimeError(f"GitHub get error {r.status_code}: {r.text[:160]}")

def _gh_put_contents(path, content_bytes, message, sha=None):
    url = f"https://api.github.com/repos/{GH_REPO}/contents/{path}"
    body = {"message": message, "content": base64.b64encode(content_bytes).decode(), "branch": GH_BRANCH}
    if sha: body["sha"] = sha
    r = requests.put(url, headers=_gh_headers(), json=body, timeout=20)
    if r.status_code not in (200,201):
        raise RuntimeError(f"GitHub put error {r.status_code}: {r.text[:160]}")
    return r.json()

def gh_append_jsonl(path, obj:dict, retries=4, wait=0.8):
    if not (GITHUB_TOKEN and GH_REPO): return
    line = (json.dumps(obj, ensure_ascii=False) + "\n").encode("utf-8")
    for attempt in range(retries):
        content, sha = _gh_get_contents(path)
        new_content = content + line
        try:
            _gh_put_contents(path, new_content, f"append {os.path.basename(path)} ({obj.get('type','event')})", sha)
            return
        except Exception:
            time.sleep(wait * (attempt+1))
    st.warning("GitHub 동기화 재시도 실패 (로컬 저장은 완료됨).")

# ─────────── SQLite (전역 공유 + WAL) ───────────
DB_PATH = os.path.join(".", "classdb.db")
...
def db_upsert_student(student_id, year, school, grade, klass, number, name):
    conn = _sqlite_conn()
    conn.execute("""INSERT INTO students(student_id, year, school, grade, klass, number, name, created_at)
                    VALUES (?,?,?,?,?,?,?,?)
                    ON CONFLICT(student_id) DO UPDATE SET
                      year=excluded.year, school=excluded.school, grade=excluded.grade,
                      klass=excluded.klass, number=excluded.number, name=excluded.name;""",
                 (student_id, year, school, grade, klass, number, name, datetime.datetime.now().isoformat()))
    conn.commit()
    # GitHub 학생 로그 append
    gh_append_jsonl(GH_STUDENTS_PATH, {"type":"student","student_id":student_id,"year":year,"school":school,
                      "grade":grade,"klass":klass,"number":number,"name":name,"ts":datetime.datetime.now().isoformat()})
...
def db_save_event(student_id, page, payload_dict):
    ...
    conn.commit()
    gh_append_jsonl(GH_EVENTS_PATH, {"type":"event","student_id":student_id,"ts":datetime.datetime.now().isoformat(),
                                     "page":page,"payload":payload_dict})
...
def rewrite_polite(text:str)->str:
    try:
        if not OPENAI_API_KEY: return "바르고 고운말을 사용해 다시 표현해 보세요."
        return client.chat.completions.create(model="gpt-4.1",
                messages=[{"role":"user","content":f"다음 문장을 초등학생에게 어울리는 바르고 고운말로 바꿔줘. 의미는 유지하고 공격적 표현은 모두 제거:\n{text}"}],
                temperature=0.2,max_tokens=120).choices[0].message.content.strip()
    except: return "바르고 고운말을 사용해 다시 표현해 보세요."

# ─────────── OpenAI/Naver Books/OCR ───────────
def gpt(msg,t=0.5,mx=800):
    if not OPENAI_API_KEY: return "(OPENAI_API_KEY 미설정)"
...
def nv_search(q):
    ...
    try:
        res=requests.get("https://openapi.naver.com/v1/search/book.json",headers=hdr,params={"query":q,"display":10},timeout=10)
        res.raise_for_status()
        return [b for b in res.json().get("items",[]) if not is_adult_book(b)]
    except: return []
...
def ensure_student_id():
    ...
    school= st.session_state.get("school", "한국초")
    grade = st.session_state.get("grade", 3)
    klass = st.session_state.get("klass", 1)
    number= st.session_state.get("number", 1)
    name  = st.session_state.get("name", "")
    student_id = f"{year}-{school}-{grade}-{klass}-{number}"
    st.session_state.student_id = student_id
    db_upsert_student(student_id, year, school, grade, klass, number, name)
    return student_id
...
def recommend_topics(title, syn, level, avoid:list, tries=2):
    base_prompt=(f"너는 초등 독서토론 교사야. 아래 책 '{title}'의 줄거리를 바탕으로 "
                 f"초등학생이 토론하기 좋은 **일반적인 찬반 토론 주제 2가지**를 추천해줘. "
                 f"각 주제는 반드시 **'…해야 한다.'** 또는 **'…하는 것이 옳다.'** 로 끝나는 문장형으로 써. "
                 f"질문형 금지.\nJSON 배열로만 출력\n\n줄거리:\n{syn[:1600]}")
    for _ in range(tries):
        raw=gpt([{"role":"user","content":base_prompt}],0.5,360)
        try:
...
def select_book_and_build(sel):
    ...
    st.session_state.synopsis=base_syn
...
def student_panel():
    ...
    with col1:
        st.session_state.year  = st.number_input("학년도", min_value=2020, max_value=2100, value=st.session_state.get("year", datetime.datetime.now().year), step=1)
        st.session_state.school= st.text_input("학교", value=st.session_state.get("school","한국초"))
        st.session_state.grade = st.number_input("학년", min_value=1, max_value=6, value=st.session_state.get("grade",3), step=1)
    with col2:
        st.session_state.klass = st.number_input("반", min_value=1, max_value=20, value=st.session_state.get("klass",1), step=1)
        st.session_state.number= st.number_input("번호", min_value=1, max_value=50, value=st.session_state.get("number",1), step=1)
        st.session_state.name  = st.text_input("이름(선택)", value=st.session_state.get("name",""))
    sid = ensure_student_id()
    st.success(f"현재 학생: {sid}" + (f" ({st.session_state.get('name')})" if st.session_state.get("name") else ""))
...
def page_book():
    ...
    if st.session_state.get("show_reco", False):
        ...
        with c1:
            if st.button("🔎 추천 불러오기"):
                ...
                db_save_event(st.session_state.student_id, "reco_load", {"grade": int(g), "count": len(st.session_state.get("reco",[]))})
    ...
    if st.button("🔍 검색") and q.strip():
        ...
        db_save_event(st.session_state.student_id, "search", {"q": q.strip(), "count": len(result)})
    ...
    if bk:=st.session_state.get("selected_book"):
        title=clean_html(bk["title"]); cover=bk.get("image"); syn=st.session_state.synopsis
        st.subheader("📖 줄거리"); st.write(syn or "(줄거리 없음)")
        lc,rc=st.columns([1,1])
        with lc:
            if cover: st.image(cover,caption=title,use_container_width=True)
        with rc:
            st.markdown("### 🖼️ 표지 챗봇 (독서 전 활동)")
            if "chat" not in st.session_state:
                st.session_state.chat=[
                    {"role":"system","content":f"초등 대상 표지 대화 챗봇. 난이도:{st.session_state.level}. {level_params(st.session_state.level)['language']}로 질문"},
                    {"role":"user","content":"표지입니다."},
                    {"role":"assistant","content":"책 표지에서 가장 먼저 보이는 것은 무엇인가요?"}]
            for m in st.session_state.chat:
                who="assistant" if m["role"]=="assistant" else "user"
                st.chat_message(who).write(m["content"])
            if u:=st.chat_input("답/질문 입력…"):
                if contains_bad_language(u):
                    st.warning("바르고 고운말을 사용해 주세요."); st.info(rewrite_polite(u))
                else:
        ...
        if st.button("다음 단계 ▶ 2) 낱말 탐정"):
            st.session_state.current_page="단어 알아보기"; 
            db_save_event(st.session_state.student_id,"page_enter",{"page":"단어 알아보기"})
            st.rerun()
...
def page_vocab():
    ...
    if st.button("🧠 단어 알아보기"):
        if not word.strip(): st.warning("단어를 입력하세요.")
            ...
            db_save_event(st.session_state.student_id,"vocab",{"word": word})

    if st.session_state.get("vocab_meaning"):
        st.markdown("#### 뜻과 예시"); st.write(st.session_state.vocab_meaning); st.divider()
    ...
    if st.session_state.get("rel_out"):
        ...
        cL, cR = st.columns(2)
        with cL:
            st.markdown("**비슷한 말(동의어)**"); st.write(", ".join(rel.get("synonyms",[])) or "(없음)")
        with cR:
            st.markdown("**반대되는 말(반의어)**"); st.write(", ".join(rel.get("antonyms",[])) or "(없음)")
        st.markdown("**쉬운 설명**"); st.write(rel.get("meaning",""))
        if rel.get("examples"):
            st.markdown("**예문**"); 
    ...
    if st.button("다음 단계 ▶ 3) 이야기 퀴즈"):
        ...
        db_save_event(st.session_state.student_id,"page_enter",{"page":"독서 퀴즈"})
        st.rerun()
...
def page_quiz():
    ...
    if "quiz" not in st.session_state and st.button("🧠 퀴즈 생성"):
        ...
        if q: 
            ...
            db_save_event(st.session_state.student_id,"quiz_gen",{"title":title})
        else: 
            st.error("형식 오류, 다시 생성"); st.code(raw)
    ...
    if q:=st.session_state.get("quiz"):
        ...
        with c2:
            if st.button("🔁 다시 도전하기"):
                st.session_state.answers={}; st.session_state.ans_uid = uid + 1; st.rerun()

    if st.button("다음 단계 ▶ 4) 독서 생각 나누기"):
        st.session_state.current_page="독서 토론"; 
        db_save_event(st.session_state.student_id,"page_enter",{"page":"독서 토론"})
        st.rerun()

# ─────────── PAGE 4 : 독서 토론 (텍스트 전용, 자동 기록) ───────────
def page_discussion():
    ...
    with b2:
        if st.button("다음 단계 ▶ 5) 독서감상문 고치기"): 
            st.session_state.current_page="독서 감상문 피드백"; 
            db_save_event(st.session_state.student_id,"page_enter",{"page":"독서 감상문 피드백"}); st.rerun()
    ...
    if start_clicked:
        if not topic or not topic.strip(): st.warning("토론 주제를 입력하거나 선택해주세요.")
            ...
            st.session_state.update({
                ...
                "debate_chat":[{"role":"system","content":f"초등 독서토론 진행자. 난이도:{lv}, 어조:{lvp['language']}. 주제 '{topic}'. 근거는 선택 책 줄거리만 사용:\n{syn[:1200]}"}]
            })
            db_save_event(st.session_state.student_id,"debate_start",{"title": title,"topic": topic,"side": side})
            st.rerun()
    ...
    if st.session_state.get("debate_started"):
        ...
        for m in st.session_state.debate_chat:
            who="assistant" if m["role"]=="assistant" else "user"
            st.chat_message(who).write(str(m["content"]))
        rd=st.session_state.debate_round; order=st.session_state.debate_order
        if rd<=len(order):
            step=order[rd-1]; st.markdown(f"### 현재: {lbl[step]}")
            ...
            if user_turn:
                ...
                if txt:
                    if contains_bad_language(txt):
                        st.warning("바르고 고운말을 사용해 주세요."); st.info(rewrite_polite(txt))
                    else:
            ...
            if "debate_eval" not in st.session_state:
                ...
                fb_prompt=(f"너는 초등 토론 코치야. 아래 '학생 발언'만 근거로 서술형 피드백을 써줘.\n"
                ...
                db_save_event(st.session_state.student_id,"debate",{
                    ...
                    "pro_total": st.session_state.score_json.get("pro",{}).get("total",0),
                    "con_total": st.session_state.score_json.get("con",{}).get("total",0),
                    "winner": st.session_state.score_json.get("winner","-")
                })
...
def page_feedback():
    ...
    if up and st.session_state.get("ocr_file")!=up.name:
        st.session_state.essay=nv_ocr(up.read()); st.session_state.ocr_file=up.name
        db_save_event(st.session_state.student_id,"ocr",{"source_file": up.name, "length": len(st.session_state.essay or "")})
        st.rerun()
...
def page_portfolio_dashboard():
    ...
    school= col2.text_input("학교", value=st.session_state.get("school","한국초"))
...
def main():
    ...
    with st.sidebar:
        ...
        # 페이지 진입 자동 기록
        try: db_save_event(st.session_state.student_id, "page_enter", {"page": st.session_state.current_page})
        except: pass
